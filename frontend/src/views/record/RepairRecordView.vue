<template>
  <div>
    ...
    <el-dialog v-model="dialog" :title="form.id?'编辑维修记录':'新增维修记录'">
      <el-form :model="form" label-width="100px"><el-form-item label="工单ID"><el-input v-model="form.repairOrderId"/></el-form-item><el-form-item label="设备ID"><el-input v-model="form.deviceId"/></el-form-item><el-form-item label="故障原因"><el-input v-model="form.faultReason"/></el-form-item><el-form-item label="处理过程"><el-input type="textarea" v-model="form.processDetail"/></el-form-item><el-form-item label="处理结果"><el-input type="textarea" v-model="form.resultDetail"/></el-form-item><el-form-item label="是否解决"><el-select v-model="form.isResolved"><el-option label="是" :value="1"/><el-option label="否" :value="0"/></el-select></el-form-item></el-form>
...
const query=reactive({deviceId:'',maintainerId:''}),page=reactive({current:1,size:10}),list=ref([]),total=ref(0),dialog=ref(false),form=reactive({})
const load=async()=>{const r=await getPage('/repair-records/page',{...query,...page});list.value=r.records;total.value=r.total}
const reset=()=>{query.deviceId='';query.maintainerId='';load()}
const openAdd=()=>{Object.assign(form,{id:null,repairOrderId:'',deviceId:'',faultReason:'',processDetail:'',resultDetail:'',isResolved:1});dialog.value=true}
