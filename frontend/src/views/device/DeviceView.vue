<template>
  <div>
    ...
    <el-dialog v-model="dialog" :title="mode==='view'?'设备详情档案':(form.id?'编辑设备':'新增设备')" width="900px">
      <template v-if="mode==='view'">
        <el-row :gutter="12" style="margin-bottom: 12px">
          <el-col :span="6"><el-card shadow="never"><div>累计报修</div><h3>{{ profile.totalOrders || 0 }}</h3></el-card></el-col>
          <el-col :span="6"><el-card shadow="never"><div>累计维修</div><h3>{{ profile.totalRepairs || 0 }}</h3></el-card></el-col>
          <el-col :span="6"><el-card shadow="never"><div>平均工时(h)</div><h3>{{ profile.avgRepairHours || 0 }}</h3></el-card></el-col>
          <el-col :span="6"><el-card shadow="never"><div>高故障预警</div><el-tag :type="profile.isHighFault?'danger':'success'">{{ profile.isHighFault?'是':'否' }}</el-tag></el-card></el-col>
        </el-row>
      </template>
      ...
      <template v-if="mode==='view'">
        ...
        <el-empty v-if="!Object.keys(profile.faultReasonStats||{}).length" description="暂无故障原因统计"/>
