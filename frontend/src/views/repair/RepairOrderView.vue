...
const canAction = (row, action) => {
  if (action === 'ADMIN_APPROVE') return can('repair:audit') && row.status === '已提交'
  if (action === 'ADMIN_REJECT') return can('repair:reject') && row.status === '已提交'
  ...
  if (action === 'MAINTAINER_DELAY_APPLY') return can('repair:delay:apply') && row.status === '维修中'
  if (action === 'MAINTAINER_PARTS_APPLY') return can('repair:parts:apply') && row.status === '维修中'
  if (action === 'MAINTAINER_FINISH') return can('repair:finish') && row.status === '维修中'
  if (action === 'USER_CONFIRM_RESOLVED' || action === 'USER_CONFIRM_UNRESOLVED') return can('repair:confirm') && row.status === '待验收'
  return false
}
